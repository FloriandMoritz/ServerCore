...
public class UnmutelogCommand extends Command {

    private final ServerCore serverCore;

    public UnmutelogCommand(final ServerCore serverCore) {
        super("unmutelog");
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String s, @NotNull String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            final CustomWindow formWindowCustom = new CustomWindow("§7» §8Unmutelog");
            ...
            formWindowCustom.onSubmit((e, r) -> {
                ...
                this.serverCore.getTinyRabbit().sendAndReceive((delivery -> {
                    switch (GuardianCalls.valueOf(delivery.getKey().toUpperCase())) {
                        case CALLBACK_ENTRIES_NOT_FOUND -> sender.sendMessage(Language.get("guardian.mute.log.no.entries"));
                        case CALLBACK_UNMUTE_BEHAVIOR_ENTRIES_RECEIVED -> {
                            ...
                            final SimpleWindow form = new SimpleWindow.Builder("§7» §8Unmutelog", content.toString()).build();
                            form.send(e);
                        }
                    }
