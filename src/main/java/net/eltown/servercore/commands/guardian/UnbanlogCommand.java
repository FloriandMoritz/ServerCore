...
public class UnbanlogCommand extends Command {

    private final ServerCore serverCore;

    public UnbanlogCommand(final ServerCore serverCore) {
        super("unbanlog");
        this.serverCore = serverCore;
        this.setDescription("Unbanlog Command");
        this.setPermission("core.command.unbanlog");
    }
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String s, @NotNull String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            final CustomWindow formWindowCustom = new CustomWindow("§7» §8Unbanlog");
            ...
            formWindowCustom.onSubmit((e, r) -> {
                ...
                this.serverCore.getTinyRabbit().sendAndReceive((delivery -> {
                    switch (GuardianCalls.valueOf(delivery.getKey().toUpperCase())) {
                        case CALLBACK_ENTRIES_NOT_FOUND -> sender.sendMessage(Language.get("guardian.ban.log.no.entries"));
                        case CALLBACK_UNBAN_BEHAVIOR_ENTRIES_RECEIVED -> {
                            ...
                            list.forEach(z -> {
                                this.serverCore.getLogger().info(z);
                            ...
                            final SimpleWindow form = new SimpleWindow.Builder("§7» §8Unbanlog", content.toString()).build();
                            form.send(e);
                        }
                    }
                }), Queue.GUARDIAN, GuardianCalls.REQUEST_GET_UNBAN_BEHAVIOR_ENTRIES.name(), target);
            });
